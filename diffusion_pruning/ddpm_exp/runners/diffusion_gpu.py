...
class Diffusion(object):
    def __init__(self, args, config, device=None):
        ...
        self.device = device
        print("Diffusion on device", self.device )
    ...
    def build_model(self):
        ...
        if args.load_pruned_model is not None:
            ...
            # print("states.keys", states[0].keys)
    ...
    def train(self, kd=False):
        ...
        self.model = model
        model.to(self.device)
        accelerator.state.device = self.device
    ...
    def sample_image(self, x, model, last=True):
        ...
        if self.args.sample_type == "generalized":
            if self.args.skip_type == "uniform":
                ...
                raise NotImplementedError
            from functions.denoising_gpu import generalized_steps

            xs = generalized_steps(x, seq, model, self.betas, eta=self.args.eta, device = self.device)
            x = xs
            ...
            if self.args.skip_type == "uniform":
                ...
                raise NotImplementedError
            from functions.denoising_gpu import ddpm_steps

            x = ddpm_steps(x, seq, model, self.betas, device = self.device)
        else:
            raise NotImplementedError
        if last:
            x = x[0][-1]

...
parser.add_argument("--pretrained_model_ckpt", type=str, default=None)
parser.add_argument("--save_path", type=str, required=True)
parser.add_argument("--pruning_ratio", type=float, default=0.3)
parser.add_argument("--batch_size", type=int, default=128)
parser.add_argument("--device", type=str, default='cpu')
parser.add_argument("--pruner", type=str, default='taylor', choices=['taylor', 'random', 'magnitude','l1norm', 'reinit', 'diff-pruning'])
...
if __name__=='__main__':
    ...
    if os.path.isdir(args.model_path):
        if args.pretrained_model_ckpt is not None:
            print("Loading pretrained model from {}".format(args.pretrained_model_ckpt))
            unet = torch.load(args.pretrained_model_ckpt).eval()
        else:
    ...
    if 'cifar' in args.model_path:
        ...
        example_inputs = {'sample': torch.randn(1, 3, 64, 64).to(args.device), 'timestep': torch.ones((1,)).long().to(args.device)}
    ...
    if args.pruning_ratio>0:
        ...
        #    if isinstance(m, AttnDownBlock2D):
        ...
        #        channel_groups[m.query] = m.heads
        #        channel_groups[m.key] = m.heads
        #        channel_groups[m.value] = m.heads

...
def convert_ddpm_checkpoint(checkpoint, config):
    ...
    new_checkpoint["time_embedding.linear_1.weight"] = checkpoint["model.temb.dense.0.weight"]
    new_checkpoint["time_embedding.linear_1.bias"] = checkpoint["model.temb.dense.0.bias"]
    new_checkpoint["time_embedding.linear_2.weight"] = checkpoint["model.temb.dense.1.weight"]
    new_checkpoint["time_embedding.linear_2.bias"] = checkpoint["model.temb.dense.1.bias"]

    new_checkpoint["conv_norm_out.weight"] = checkpoint["model.norm_out.weight"]
    new_checkpoint["conv_norm_out.bias"] = checkpoint["model.norm_out.bias"]

    new_checkpoint["conv_in.weight"] = checkpoint["model.conv_in.weight"]
    new_checkpoint["conv_in.bias"] = checkpoint["model.conv_in.bias"]
    new_checkpoint["conv_out.weight"] = checkpoint["model.conv_out.weight"]
    new_checkpoint["conv_out.bias"] = checkpoint["model.conv_out.bias"]
    ...
    for i in range(num_down_blocks):
        ...
        if any("downsample" in layer for layer in down_blocks[i]):
            new_checkpoint[f"down_blocks.{i}.downsamplers.0.conv.weight"] = checkpoint[
                f"model.down.{i}.downsample.conv.weight"
            ]
            new_checkpoint[f"down_blocks.{i}.downsamplers.0.conv.bias"] = checkpoint[f"model.down.{i}.downsample.conv.bias"]
    ...
    for i in range(num_up_blocks):
        ...
        if any("upsample" in layer for layer in up_blocks[i]):
            new_checkpoint[f"up_blocks.{block_id}.upsamplers.0.conv.weight"] = checkpoint[
                f"model.up.{i}.upsample.conv.weight"
            ]
            new_checkpoint[f"up_blocks.{block_id}.upsamplers.0.conv.bias"] = checkpoint[f"model.up.{i}.upsample.conv.bias"]

...
def matrix_sqrt_newton(A, max_iter=10):
    X = np.eye(A.shape[0])  # Initial guess
...
def calculate_frechet_distance(mu1, sigma1, mu2, sigma2, eps=1e-6):
    ...
    #print("mu1, mu2, sigma1, sigma2,  mu1.shape, mu2.shape, sigma1.shape ,sigma2.shape",mu1, mu2, sigma1, sigma2, mu1.shape, mu2.shape, sigma1.shape ,sigma2.shape)
    ...
    # product_matrix = sigma1.dot(sigma2)
    # regularized_matrix = product_matrix + eps * np.eye(product_matrix.shape[0])

    # # Compute the square root of the regularized matrix
    # covmean, _ = linalg.sqrtm(regularized_matrix, disp=False)

    # covmean, _ = linalg.sqrtm(sigma1.dot(sigma2), disp=False)

    # offset = np.eye(sigma1.shape[0]) * eps
    # print("offset",offset)
    # regularized = (sigma1 + offset).dot(sigma2 + offset)
    # print("regularized", regularized)

    # Regularization term
    offset = np.eye(sigma1.shape[0]) * eps

    # Regularized covariance product
    regularized = (sigma1 + offset).dot(sigma2 + offset)

    # Compute the eigenvalue decomposition of the regularized covariance product
    eigvals, eigvecs = np.linalg.eig(regularized)

    # Take the square root of the eigenvalues
    sqrt_eigvals = np.sqrt(np.maximum(eigvals, 0))

    # Construct the square root matrix using the eigenvectors and square root of eigenvalues
    covmean = np.dot(eigvecs, np.dot(np.diag(sqrt_eigvals), eigvecs.T))

    # covmean = linalg.sqrtm(regularized)
    
    # # Compute eigenvalue decomposition
    # eigenvalues, eigenvectors = np.linalg.eig(regularized)
    # print("eigen done")

    # # Compute pseudo square root using eigenvalues
    # sqrt_eigenvalues = np.sqrt(eigenvalues)
    # print("eigen squareRoot done")
    # covmean = eigenvectors.dot(np.diag(sqrt_eigenvalues)).dot(eigenvectors.T)

    # # Compute singular value decomposition
    # U, S, Vt = np.linalg.svd(regularized)

    # # Compute pseudo square root using singular values
    # sqrt_singular_values = np.sqrt(S)
    # covmean = U.dot(np.diag(sqrt_singular_values)).dot(Vt)

    # alpha = 0.5  # Approximate square root power
    # covmean = np.linalg.matrix_power(regularized, alpha)

    # covmean = matrix_sqrt_newton(regularized)

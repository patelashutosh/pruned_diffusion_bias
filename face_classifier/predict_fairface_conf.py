...
def detect_face(imgs_path, SAVE_DETECTED_AT, default_max_size=800, size=300, padding=0.25):
    cnn_face_detector = dlib.cnn_face_detection_model_v1('dlib_models/mmod_human_face_detector.dat')
    sp = dlib.shape_predictor('dlib_models/shape_predictor_5_face_landmarks.dat')
    base = 2000  # largest width and height
    img_names = [os.path.join(imgs_path, x) for x in os.listdir(imgs_path)]
    for index, image_path in enumerate(img_names):
        if index % 10 == 0:
            print('---%d/%d---' %(index, len(img_names)))
...
def getConfidence(score):
    #if score is greater than 0.9 threshold, return "High", 0.8 return "Medium", else "Low"
    if np.max(score) > 0.9:
        return "4"
    elif np.max(score) > 0.8:
        return "3"
...
def predidct_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    race_scores_fair_4 = []
    race_preds_fair_4 = []
    gender_confidence_fair = []
    age_confidence_fair = []
    race_confidence_fair = []
    ...
    for index, img_name in enumerate(img_names):
        ...
        gender_confidence = getConfidence(gender_score)
        race_4_confidence = getConfidence(race_score)
        age_confidence = getConfidence(age_score)
        gender_confidence_fair.append(gender_confidence)
        age_confidence_fair.append(age_confidence)
        race_confidence_fair.append(race_4_confidence)
    ...
    # age
    result.loc[result['age_preds_fair'] == 0, 'age'] = '0-02y'
    result.loc[result['age_preds_fair'] == 1, 'age'] = '03-09y'
    result.loc[result['age_preds_fair'] == 2, 'age'] = '10-19y'
    result.loc[result['age_preds_fair'] == 3, 'age'] = '20-29y'
    result.loc[result['age_preds_fair'] == 4, 'age'] = '30-39y'
    result.loc[result['age_preds_fair'] == 5, 'age'] = '40-49y'
    result.loc[result['age_preds_fair'] == 6, 'age'] = '50-59y'
    result.loc[result['age_preds_fair'] == 7, 'age'] = '60-69y'
    result.loc[result['age_preds_fair'] == 8, 'age'] = '70y+'
    ...
    result[['face_name_align',
            ...
            'gender_confidence_fair', 'age_confidence_fair',
            'race_confidence_fair']].to_csv(save_prediction_at, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_dir', dest='image_dir', action='store',
                        help='image directory')
    parser.add_argument('--detected_faces_dir', dest='detected_faces_dir', action='store',
                        help='detected faces directory')
    parser.add_argument('--output_csv', dest='output_csv', action='store',
                        help='output csv file')
    dlib.DLIB_USE_CUDA = True
    print("using CUDA?: %s" % dlib.DLIB_USE_CUDA)
    args = parser.parse_args()
    SAVE_DETECTED_AT = args.detected_faces_dir
    ensure_dir(SAVE_DETECTED_AT)
    # imgs = pd.read_csv(args.input_csv)['img_path']
    detect_face(args.image_dir, SAVE_DETECTED_AT)
